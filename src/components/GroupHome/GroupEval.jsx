...
function getWeekRange() {
  ...
  // startDate, endDate
  const startDate = calTime(monday);
  const endDate = calTime(sunday);

  // 이번 주 일요일 10:00:00 (로컬 기준)
  const sundayForCreated = new Date(sunday);
  sundayForCreated.setHours(10, 0, 0, 0);

  const thisSunday =
    sundayForCreated.getFullYear() +
    "-" +
    String(sundayForCreated.getMonth() + 1).padStart(2, "0") +
    "-" +
    String(sundayForCreated.getDate()).padStart(2, "0") +
    "T" +
    String(sundayForCreated.getHours()).padStart(2, "0") +
    ":" +
    String(sundayForCreated.getMinutes()).padStart(2, "0") +
    ":" +
    String(sundayForCreated.getSeconds()).padStart(2, "0");

  return { startDate, endDate, thisSunday };
}
...
const GroupEval = () => {
  ...
  const { startDate, endDate, thisSunday } = getWeekRange();
  ...
  useEffect(() => {
    ...
    const fetchPersonData = async () => {
      try {
        ...
        const res2 = await axiosInstance.get("/groups/evaluation-view/");
  ...
  // 평점 누적시킬 리스트 만듦
  useEffect(() => {
    const lastRatingInfo = JSON.parse(localStorage.getItem("lastRatingInfo"));
  ...
  const onClickSave = async () => {
    ...
    try {
      ...
      const res = await axiosInstance.post("/groups/evaluation/", {
        created_at: thisSunday, // "2025-07-27T10:00:00" 테스트용 데이터 (일요일일때만 post 가능하대, 안그러면 400 에러뜨게 로직짜뒀대)
