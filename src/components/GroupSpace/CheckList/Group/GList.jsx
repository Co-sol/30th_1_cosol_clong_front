...
import PListAddModal from "../Person/PListAddModal";
  ...
  return (
    <div className="GList">
      ...
      <div className="scrollBar">
        ...
        {isEditMode && <Button onClick={onClickAdd} text={"+"} type={"add1"} />}
        {/* {isAddMode && (
                    <GListAddModal
        ...
        {isAddMode && !selectedData && (
          <GListAddModal
            ...
            selectedPlace={selectedPlace}
            personData={personData}
            addCheckItem={(item) => setCheckListData((prev) => [...prev, item])}
          />
        )}
        {isAddMode && selectedData && (
          <PListAddModal
            isAddMode={isAddMode}
            setIsAddMode={setIsAddMode}
            targetPlaceData={[]} // 빈 배열 또는 적절한 데이터
            selectedName={selectedData.owner}
            selectedBadgeId={1} // 적절한 badgeId
            selectedParentPlace={selectedData.name}
            clickedDiagram={{ space_name: selectedPlace }}
